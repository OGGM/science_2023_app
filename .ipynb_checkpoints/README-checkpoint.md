# Global and regional future evolution of glaciers

OGGM edu graphic: illustrates the evolution of the volume of the all glaciers world-wide by using the data of [Rounce, D.R., R. Hock, and F. Maussion. 2022. Global PyGEM-OGGM Glacier Projections with RCP and SSP Scenarios, Version 1](https://doi.org/10.5067/P8BN9VO9N5C7).

(for more information, visit [edu.oggm.org](https://edu.oggm.org))

Author:

- [Lilian Schuster](https://github.com/lilianschuster) (University of Innsbruck)
- [Fabien Maussion](https://github.com/fmaussion) (University of Innsbruck)

License: BSD3

[![badge](https://img.shields.io/badge/launch-OGGM%20Edu%20app-cc514e.svg?logo=data:image/png;base64,)](https://edu.oggm.org/en/latest/science_2023_app.html)

## Possible future improvements:
- implementation of uncertainties into the graphic
- possibility to choose between absolute and percentage change of volume
- adding a map in which the user can select RGI regions
- add other informations, such as amount of glaciers, future projected regional glacier temperature change, ... 
- implement tick-boxes for choosing the displayed RCP-scenarios
- two dropdown menus to give the opportunity that every region can be compared to every other region

If you are interested to contribute you can send us the edited jupyter-notebook per mail or with a pull-request. Thanks!
